...
const AdminItem = ({items : {catalog_ID, catalog_name, catalog_price, catalog_description, catalog_image}, onReload, category_ID } ) => {
  ...
  const updateItem = async (e) => {
        ...
        try {
          ...
          const response = await axios.put(`http://localhost:8083/catalog/${catalog_ID}/category/${category_ID}`, inputData, {
              headers: {
  ...
  return (
    <div className="item_card">
        ...
        <button className="editCatalog" onClick={() => openUpdateForm({catalog_ID})}><img src="/edit-icon.svg" className="editIcon2"/></button>
        ...
        <div className="editItemForm" style={isActive ? {display: "flex"} : {display: "none"}}>
            ...
            <button className="closeBtn2" onClick={openUpdateForm}>x</button>
              <form onSubmit={updateItem}>
                ...
                <input className="fields" type="file" placeholder="Item Image" name="catalog_image" onChange={handleChange} /><img className="catalog_image" src={`http://localhost:8083/images/${catalog_image}`}/>
